...
ADMIN_USER_IDS = [1234567890]    # Replace with your Telegram user ID(s)
# ==================================
...
class TradingBot:
    ...
    def create_trade(self, pair, trade_type, current_price=None):
        ...
        # Determine trade amount (fixed or percentage of balance)
        trade_amount = self.config['amount']  # Default to fixed amount
        
        # If using percentage and we have API access, calculate based on balance
                ...
                if account_info:
                    ...
                    for asset_balance in account_info.get('balances', []):
                        if asset_balance['asset'] == base_asset:
                            ...
                            # Use percentage amount if it's valid
                            if percentage_amount > 0:
                                trade_amount = percentage_amount
                                logger.info(f"Using {self.config['trade_percentage']}% of {base_asset} balance: {trade_amount}")
                            break
            except Exception as e:
