...
ADMIN_USER_IDS = [12394779]    # Replace with your Telegram user ID(s)
# ==================================

# Binance API configuration
BINANCE_API_KEY = "API_KEY"  # Your Binance API key
BINANCE_API_SECRET = "API_SECRECT" 
...
TRADING_MODES = {
    "safe": {
        "take_profit": 1.5,  # 1% take profit
        "stop_loss": 5.0,    # 0.5% stop loss
    ...
    "standard": {
        "take_profit": 2.5,  # 2% take profit
        "stop_loss": 7.5,    # 1% stop loss
...
CONFIG = {
    ...
    "amount": 0.01,           # Amount of BNB to trade
    ...
    "mock_mode": False           # Set to False to disable mock data and use only real data
}
...
class WhaleDetector:
            ...
            try:
                ...
                if random.random() < 0.1 and self.config["whale_detection"]:  # 10% chance each cycle
                    ...
                    if current_time - self.last_notification_time > 30:
                        ...
                        # Queue the notification for sending by the main bot
                        self.trading_bot.queue_notification(whale_message, keyboard)
    ...
    def process_whale_for_trading(self, whale_transaction):
        ...
        self.trading_bot.create_trade_from_whale(whale_transaction, trade_type)
...
class TradingBot:
    ...
    def queue_notification(self, message, keyboard=None):
        """Queue a notification to be sent asynchronously"""
        # Add the notification to the queue
        if self.telegram_bot and self.telegram_bot.admin_chat_ids:
            for chat_id in self.telegram_bot.admin_chat_ids:
                self.notification_queue.put_nowait((chat_id, message, keyboard))
    ...
    async def process_notification_queue(self):
        ...
        while True:
            try:
                ...
                chat_id, message, keyboard = await self.notification_queue.get()
    ...
    def apply_trading_mode_settings(self):
        ...
        if mode in TRADING_MODES:
            ...
            self.config["price_change_threshold"] = mode_settings["price_change_threshold"]
                ...
                if self.config["auto_select_pairs"]:
                    ...
                    if best_pairs:
                        ...
                        if random.random() < 0.3 and self.config["trading_enabled"]:  # 30% chance to create a trade
                            trade = self.create_trade(pair_name, trade_type, selected_pair["last_price"])
                            ...
                            entry_message = (
                                ...
                                f"Selection: Auto (Volume: {selected_pair['volume']:.2f}, Change: {selected_pair['price_change']:.2f}%)"
                            )
                            
                            # Queue the notification
                            self.queue_notification(entry_message)
    ...
    def create_trade_from_whale(self, whale_transaction, trade_type):
        ...
        entry_message = (
            ...
            f"Based on whale transaction: {whale_transaction['id']}"
        )
    ...
    def complete_trade(self, trade, exit_price=None, reason="unknown"):
        ...
        complete_message = (
            ...
            f"Strategy: {trade.get('strategy', 'Standard Trading')}\n"
            f"Real Trade: {'Yes' if trade.get('real_trade', False) else 'No (Simulation)'}"
        )
        
        # Queue the notification
        self.queue_notification(complete_message)
...
class TelegramBotHandler:
    def __init__(self, token, admin_ids):
        ...
        self.admin_chat_ids = []  # Will be populated when admins interact with the bot
    ...
    def register_handlers(self):
        ...
        # Add this to the register_handlers method:
    ...
    async def test_api_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if self.trading_bot.binance_api:
            ...
            try:
                ...
                # Show which API we're connected to
                ...
                # Now try to get account info to test authentication
    ...
    async def toggle_testnet_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Toggle testnet setting
        self.trading_bot.config["use_testnet"] = not self.trading_bot.config["use_testnet"]
        
        # Recreate the Binance API instance with the new setting
    ...
    async def enable_real_trading_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Test the API connection before enabling real trading
        ...
        if self.trading_bot.binance_api:
            try:
                ...
                if account_info:
                    ...
                    # Sort by value (if we have price data)
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if chat_id not in self.admin_chat_ids:
            ...
            logger.info(f"Added chat ID {chat_id} to admin chats")
    ...
    async def status_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Calculate profit statistics if there are completed trades
        total_profit = 0
        win_count = 0
        loss_count = 0
        ...
        # Check if this is from a callback query or a direct command
        if update.callback_query:
    ...
    async def config_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Format the config in a more readable way
    ...
    async def set_config_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            if param in ['amount', 'take_profit', 'stop_loss', 'whale_threshold', 'min_volume', 'min_price_change']:
                value = float(value)
            elif param in ['trading_enabled', 'whale_detection', 'auto_trade_on_whale', 'safety_mode', 'auto_select_pairs', 'use_testnet', 'use_real_trading', 'mock_mode']:
                value = value.lower() in ['true', 'yes', '1', 'on']
            elif param in ['max_trade_time', 'max_concurrent_trades', 'market_update_interval']:
                value = int(value)
            elif param == 'trading_mode' and value not in TRADING_MODES:
                await update.message.reply_text(
            ...
            await update.message.reply_text(f"Invalid value for {param}: {value}")
            return
        
        # Update configuration
        self.trading_bot.config[param] = value
        
        # If API credentials changed, recreate the Binance API instance
        ...
        # If trading mode was changed, apply its settings
        if param == 'trading_mode':
            self.trading_bot.apply_trading_mode_settings()
            await update.message.reply_text(
                f"Trading mode changed to {value}. Applied new settings:\n"
                f"• Take Profit: {self.trading_bot.config['take_profit']}%\n"
                f"• Stop Loss: {self.trading_bot.config['stop_loss']}%\n"
                f"• Max Trade Time: {self.trading_bot.config['max_trade_time']} seconds\n"
            ...
            await update.message.reply_text(f"Configuration updated: {param} = {value}")
    ...
    async def trades_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        for trade in recent_trades:
            ...
            result = f"{trade.get('result', 0):.2f}%" if trade.get('completed', False) else "N/A"
            
            # Calculate time elapsed or duration
            if not trade.get('completed', False):
                elapsed = int(time.time() - trade['timestamp'])
                time_info = f"Elapsed: {elapsed}s"
            else:
                duration = int(time.time() - trade['timestamp'])
                time_info = f"Duration: {duration}s"
    ...
    async def whales_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Show the most recent 5 whale transactions
    ...
    async def bnb_pairs_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Get all pairs from market data
        pairs = self.trading_bot.market_analyzer.market_data
        
        # Separate BNB base pairs and BNB quote pairs
    ...
    async def volume_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Add buttons to trade these pairs
        keyboard = []
    ...
    async def trending_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        for i, pair in enumerate(trending_pairs, 1):
            # Add emoji based on price change direction
    ...
    async def whale_config_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Create keyboard for whale configuration
        keyboard = [
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Start trading with selected mode
        ...
        # Set trading mode without starting
        ...
        if query.data.startswith("trade_"):
            ...
            if pair_data:
                # Determine trade type based on price change
                ...
                await query.edit_message_text(
                    ...
                    f"Max Time: {trade['max_time_seconds']} seconds\n"
                    f"Mode: {self.trading_bot.config['trading_mode'].capitalize()}\n\n"
                    f"You will receive a notification when this trade completes."
                )
            else:
        ...
        if query.data == "stop_trading":
            ...
            # Set strategy to follow whale
            self.trading_bot.config["trading_strategy"] = "follow_whale"
            await self.whale_config_command(update, context)
                
        elif query.data == "strategy_counter_whale":
            # Set strategy to counter whale
            self.trading_bot.config["trading_strategy"] = "counter_whale"
            await self.whale_config_command(update, context)
                
        elif query.data == "set_whale_threshold":
            # This would ideally open a dialog to set the threshold
            # For simplicity, we'll just cycle through some preset values
            current = self.trading_bot.config["whale_threshold"]
            if current < 50:
                self.trading_bot.config["whale_threshold"] = 50
            elif current < 100:
                self.trading_bot.config["whale_threshold"] = 100
            elif current < 200:
                self.trading_bot.config["whale_threshold"] = 200
            elif current < 500:
                self.trading_bot.config["whale_threshold"] = 500
            else:
                self.trading_bot.config["whale_threshold"] = 10
            
            await self.whale_config_command(update, context)
                
        elif query.data.startswith("follow_whale_"):
            # Extract whale ID from callback data
            whale_id = int(query.data.split("_")[2])
            
            # Find the whale transaction
            whale_transaction = next((w for w in MOCK_WHALE_TRANSACTIONS if w['id'] == whale_id), None)
            
            if whale_transaction:
                # Create a trade following the whale
                self.trading_bot.create_trade_from_whale(whale_transaction, whale_transaction['type'])
                await query.edit_message_text(f"Following whale transaction {whale_id}. Trade created!")
            else:
                await query.edit_message_text(f"Whale transaction {whale_id} not found.")
                
        elif query.data.startswith("ignore_whale_"):
            # Just acknowledge the ignore action
            whale_id = int(query.data.split("_")[2])
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # For now, just respond with a help message
        await update.message.reply_text(
    ...
    def run(self):
        """Run the bot"""
        # Create a new event loop for the notification processor
        notification_loop = asyncio.new_event_loop()
        
        # Start the notification processor in a separate thread
...
def main():
    # Use the hardcoded Telegram bot token
    token = TELEGRAM_BOT_TOKEN
    
    # Initialize the Telegram bot handler with admin user IDs
    ...
    # Set the whale detector in the trading bot
